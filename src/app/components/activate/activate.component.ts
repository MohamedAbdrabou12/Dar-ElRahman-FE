...
export class ActivateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.token) {
      this.tokenError = 'رابط التفعيل غير صالح. يرجى التحقق من بريدك الإلكتروني.';
      return;
    }
    this.validateToken();
  }
    ...
    this.authService.validateActivationToken(this.token).subscribe(
      ...
      (error) => {
        ...
        if (msg.includes('expired')) {
          this.tokenError = 'انتهت صلاحية رابط التفعيل. يرجى طلب رابط جديد من الإدارة.';
        } else if (msg.includes('already')) {
          this.tokenError = 'تم تفعيل هذا الحساب مسبقاً. يمكنك تسجيل الدخول.';
        } else {
          this.tokenError = 'رابط التفعيل غير صالح. يرجى التحقق من بريدك الإلكتروني.';
        }
      }
  ...
  onSubmit(form: NgForm) {
    ...
    this.loadingService.startLoading();
    this.authService.activateAccount(this.token, this.passwords.password).subscribe(
      () => {
        this.loadingService.stopLoading();
        this.activated = true;
        this.showToast('تم تفعيل حسابك بنجاح!', 'success');
        setTimeout(() => this.router.navigate(['/', AppRoutes.LOGIN]), 2500);
      },
      (error) => {
        this.loadingService.stopLoading();
        this.showToast('حدث خطأ أثناء تفعيل الحساب. حاول مرة أخرى.', 'error');
      }
    );
  }
}
